(function () {
    ...
    var urlObj = {
        ...
        palacetuikuan: '/mobile2/dier_lesson/payBackTest', //少年宫退款
    };
    // 判断设备
    ...
    if (document.getElementById('jsClassList')) {
        ...
        $('.jz-buy-btn').click(function () {
            ...
            var wallConfirm = layer.confirm('是否报名此课程？', ['确定', '取消'], function () {
    ...
    } else if (document.getElementById('jzqddet')) {
        // 获取地址栏，如果是少年宫就插入退款按钮
        var url_type = (root.getQueryString('type'))
        var lesson_id = (root.getQueryString('id'))
        // if(url_type == 'palace') {
        //     $('#jzqddet').append(`
        //     <div class="tui-btn-bx">
        //         <div class="jz-tui-btn layui-btn layui-btn-radius layui-btn-warm ">
        //             申请退款
        //         </div>
        //     </div>
        //     `)
        // }
        $('.jz-tui-btn').click(function () {
            var url = baseUrl + urlObj.palacetuikuan
            var $self = $(this)
            // $self.off()
            // console.log(11)
            layer.open({
                ...
                title: '申请退款',
                content: `
                <div class="layui-form tk-form-bx" >
                <div class="layui-form-item tuikuan-input">
                    <div class="layui-input-block">
                        <input type="text" name="tkreason" required lay-verify="required" placeholder="请输入退款原因"
            ...
            layui.use('form', function () {
                ...
                form.on('submit(formDemo)', function (data) {
                    ...
                    var payback_reason = data.field.tkreason
                    var data = {
                        lesson_id: lesson_id,
                    ...
                    $.post(url, data, function (res) {
                        layer.msg(res.msg)
                        if (res.code == "1") {
                            $self.css('backgroundColor', "#CCC").text('待退款')
                            $self.off()
